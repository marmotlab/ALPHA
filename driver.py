...
def main():
    ...
    # Continue training or start over
    ...
    try:
        while curr_steps < TrainingParameters.N_MAX_STEPS:
            if update_done:
                ...
                demon_probs = np.random.rand()  # Evenly distributed random number
            ...
            if demon:
                ...
                for results in range(done_len):
                    ...
                    mb_graph_nodes.append(job_results[results][2])
                    mb_agent_intent.append(job_results[results][3])
                    mb_node_index.append(job_results[results][4])
                    mb_agent_index.append(job_results[results][5])
                    mb_actions.append(job_results[results][6])
                    mb_hidden_state.append(job_results[results][7])
                ...
                mb_imitation_loss = []
                # todo: fix this bug bc the start can only be 0
                for start in range(0, np.shape(mb_obs)[0], TrainingParameters.MINIBATCH_SIZE):
                    print("np.shape(mb_obs)[0]", np.shape(mb_obs))
                    print("start", start)
                    end = start + TrainingParameters.MINIBATCH_SIZE
                    slices = (arr[start:end] for arr in
                              (mb_obs, mb_vector, mb_graph_nodes, mb_agent_intent, mb_node_index, mb_agent_index, mb_actions, mb_hidden_state))
                ...
                mb_obs, mb_vector, mb_returns, mb_values, mb_actions, mb_ps, mb_hidden_state, mb_train_valid,\
                    mb_blocking = [], [], [], [], [], [], [], [], []
                mb_graph_nodes, mb_agent_intent = [], []
                mb_node_index, mb_agent_index = [], []
                ...
                for results in range(done_len):
                    ...
                    mb_node_index.append(job_results[results][4])
                    mb_agent_index.append(job_results[results][5])
                    mb_returns.append(job_results[results][6])
                    mb_values.append(job_results[results][7])
                ...
                for _ in range(TrainingParameters.N_EPOCHS):
                    ...
                    for start in range(0, done_len * TrainingParameters.N_STEPS, TrainingParameters.MINIBATCH_SIZE):
                        ...
                        slices = (arr[mb_inds] for arr in
                                  (mb_obs, mb_vector, mb_graph_nodes, mb_agent_intent, mb_node_index, mb_agent_index,
                                   mb_returns, mb_values, mb_actions, mb_ps, mb_hidden_state,
...
def evaluate(eval_env, model, device, save_gif, curr_steps, greedy):
    ...
    for i in range(RecordingParameters.EVAL_EPISODES):
        ...
        done, valid_actions, obs, vector, graph_nodes, agent_intent, node_index, agent_index, _ = reset_env(eval_env, num_agent)
        ...
        while not done:
            # predict
            actions, pre_block, hidden_state, num_invalid, v, ps = model.evaluate(obs, vector, graph_nodes,
                                                                                  agent_intent,
                                                                                  node_index, agent_index,
                                                                                  valid_actions,
                                                                                  hidden_state, greedy, num_agent)

            one_episode_perf['invalid'] += num_invalid

            # move
            rewards, valid_actions, obs, vector, graph_nodes, agent_intent, node_index, agent_index, _, done, _, \
                num_on_goals, one_episode_perf, max_on_goals, _, \
                = one_step(eval_env, one_episode_perf, actions, pre_block, num_agent)
